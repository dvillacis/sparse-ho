...
def grad_search(algo, criterion, model, optimizer, X, y, alpha0, monitor):
    """
    Parameters
    ----------
    ...
    def _get_val_grad(log_alpha, tol, monitor):
        return criterion.get_val_grad(
            model, X, y, log_alpha, algo.compute_beta_grad, tol=tol, monitor=monitor
        )
...
def hyperopt_wrapper(
    ...
    def objective(log_alpha):
        ...
        val_func = criterion.get_val(model, X, y, log_alpha, monitor, tol=tol)
        return val_func

    # TODO, also size_space = n_hyperparam ?
    space = [
        hp.uniform(str(dim), np.log(alpha_min), np.log(alpha_max))
        for dim in range(size_space)
    ]
    ...
    if method == "bayesian":
        ...
        fmin(
            objective, space, algo=algo, max_evals=max_evals, timeout=t_max, rstate=rng
        )
    elif method == "random":
        fmin(
            objective,
