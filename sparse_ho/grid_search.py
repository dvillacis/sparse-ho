...
def grid_search(
    ...
    if alphas is None and samp == "grid":
        ...
        if nb_hyperparam == 2:
            alphas = np.array(np.meshgrid(alphas, alphas)).T.reshape(-1, 2)

    elif samp == "random":
        rng = np.random.RandomState(random_state)
        # sample uniformly on log scale
        alphas = np.exp(
            rng.uniform(np.log(alpha_min), np.log(alpha_max), size=max_evals)
        )
        if reverse:
        ...
        if nb_hyperparam == 2:
            ...
            alphas = np.array(np.meshgrid(alphas, alphas2)).T.reshape(-1, 2)
    ...
    for i, alpha in enumerate(alphas):
        ...
        g_func = criterion.get_val(model, X, y, np.log(alpha), monitor, tol=tol)
